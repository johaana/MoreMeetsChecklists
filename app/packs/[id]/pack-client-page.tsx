...
export default function PackClientPage({ pack, heroImageUrl }: { pack: PremiumPack, heroImageUrl: string }) {
    ...
    return (
        <div className="bg-[#F7F8FA] text-[#0B0F14] font-sans antialiased selection:bg-[#F4A261]/30">
            ...
            <section className="relative w-full bg-[#F7F8FA] pt-12 pb-8 md:pt-20 md:pb-12 overflow-hidden border-b border-zinc-200">
                <div className="container mx-auto max-w-[1200px] px-6">
                    <div className="grid lg:grid-cols-[1.1fr,1fr] gap-10 lg:gap-16 items-center">
                        ...
                        <div className="flex flex-col items-start space-y-6 md:space-y-8 relative z-20">
                            <div className="space-y-2 md:space-y-3">
                                <span className="text-[10px] md:text-[12px] font-black uppercase tracking-[0.4em] md:tracking-[0.5em] text-[#5B6670] mb-1 block">/ {pack.category.toUpperCase()}</span>
                                <h1 className="text-[32px] md:text-[56px] lg:text-[68px] font-black text-[#0B0F14] leading-[0.85] tracking-tighter uppercase italic">
                            ...
                            <div className="border-l-[3px] border-[#0F3D2E] pl-4 md:pl-[20px] space-y-1.5 py-0.5">
                                <p className="text-sm md:text-[16px] font-bold leading-none text-[#0B0F14]">Even when you’re not there.</p>
                                <p className="text-sm md:text-[16px] font-bold leading-none text-[#0B0F14]">Without follow-ups</p>
                                <p className="text-sm md:text-[16px] font-bold leading-none text-[#0B0F14]">Without confusion</p>
                                <p className="text-sm md:text-[16px] font-bold leading-none text-[#0B0F14]">Without depending on memory</p>
                            </div>

                            <div className="flex items-center gap-4 md:gap-6 w-full text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] md:tracking-[0.25em]">
                                <span className="flex items-center gap-2 text-zinc-500 font-headline"><LayoutGrid className="w-3 md:w-3.5 h-3 md:h-3.5" /> {totalChecklists} MODULES</span>
                                <span className="flex items-center gap-2 text-zinc-500 font-headline"><Target className="w-3 md:w-3.5 h-3 md:h-3.5" /> {totalTasks}+ CHECKPOINTS</span>
                            </div>
                            ...
                            <div className="w-full pt-2">
                                <div className="flex flex-col sm:flex-row items-center gap-6">
                                    ...
                                    <div className="flex flex-col text-center sm:text-left">
                                        <p className="text-[8px] md:text-[9px] font-black text-[#0B0F14] uppercase tracking-widest leading-none">ONE-TIME PAYMENT</p>
                                        <p className="text-[8px] md:text-[9px] font-black text-[#5B6670] uppercase tracking-widest leading-none mt-1">OWN THE ENGINE FOREVER</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="relative z-10 w-full lg:pt-0 pt-4">
                            <div className="relative mx-auto group max-w-xl">
                                <div className="bg-[#111] h-8 md:h-9 w-full rounded-t-xl md:rounded-t-[14px] flex items-center px-4 gap-2 border border-white/10">
                                ...
                                <div className="mt-3 text-center">
                                    <p className="text-[6px] md:text-[7px] font-bold text-[#5B6670] uppercase tracking-[0.4em] italic opacity-60">
            ...
            <Section className="bg-white border-y border-[#E6E8EC]">
                <div className="max-w-[1000px] mx-auto text-center space-y-12 md:space-y-16">
                    <div className="space-y-4">
                        ...
                        <h2 className="text-[28px] md:text-[48px] font-black text-[#0B0F14] uppercase italic tracking-tight leading-tight">Why {pack.title.toLowerCase().replace(' operating system', '')} operations break</h2>
                    </div>
                    
                    <div className="grid md:grid-cols-2 gap-4 text-left">
                        {content.pitfalls.map((p, i) => (
                            <div key={i} className="flex items-center gap-4 md:gap-5 p-6 md:p-8 rounded-2xl bg-[#F7F8FA] border border-zinc-100 hover:border-red-100 transition-all group">
                                <div className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-red-500 shrink-0 group-hover:scale-125 transition-transform shadow-[0_0_8px_rgba(239,68,68,0.3)]" />
                                <span className="text-[#5B6670] font-bold italic text-base md:text-lg leading-tight">{p}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <Section className="bg-white">
                <div className="space-y-12 md:space-y-20">
                    <div className="text-center space-y-4 md:space-y-5">
                        ...
                        <h2 className="text-[28px] md:text-[48px] font-black text-[#0B0F14] uppercase italic tracking-tight leading-tight">What’s inside the system</h2>
                    </div>
                    
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
                        {pack.checklists.map((checklist, index) => (
                            <div key={index} className="bg-white p-8 md:p-12 rounded-[1.5rem] md:rounded-[2.5rem] space-y-6 md:space-y-8 group hover:shadow-[0_40px_100px_-15px_rgba(0,0,0,0.08)] transition-all duration-700 relative border border-zinc-100 hover:border-primary/20">
                                <div className="space-y-1 md:space-y-2">
                                    ...
                                    <h4 className="text-2xl md:text-4xl font-black italic uppercase leading-[0.9] tracking-tighter text-[#0B0F14] pt-1">
                                        {checklist.title.replace('System', '').replace('Command', '').replace('Operations', '').replace('Protocol', '')}
                                    </h4>
                                </div>
                                <div className="pt-4 flex items-center gap-3 md:gap-4 border-t border-zinc-100">
                                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-primary/10 flex items-center justify-center shadow-inner">
                                        <Target className="w-4 h-4 md:w-5 md:h-5 text-primary" />
                                    </div>
                                    <span className="text-[10px] md:text-[12px] font-black text-primary uppercase tracking-[0.3em] md:tracking-[0.4em] font-headline">
                                        {checklist.tasks.length} CHECKPOINTS
                                    </span>
                                </div>
                                <p className="text-sm md:text-[15px] text-zinc-500 italic font-medium leading-relaxed">{checklist.summary}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* 5. FINAL CTA */}
            <Section className="bg-[#0F3D2E] text-white text-center py-32 md:py-56">
                <div className="max-w-4xl mx-auto space-y-12 md:space-y-16">
                    <div className="space-y-4 md:space-y-6 px-4">
                        <h2 className="text-[36px] md:text-[80px] font-black leading-[0.9] tracking-tighter uppercase italic drop-shadow-2xl">
                            Run your <br/> {pack.title.toLowerCase().replace(' operating system', '')} <br/> properly.
                        </h2>
                        <p className="text-base md:text-[28px] text-white/60 font-bold italic pt-2">Deploy the Sovereign Master Engine in under 10 minutes.</p>
                    </div>
                    
                    <button className="h-16 md:h-[84px] px-10 md:px-16 rounded-xl md:rounded-[20px] bg-[#F4A261] text-white font-black uppercase italic text-base md:text-2xl shadow-[0_25px_60px_-10px_rgba(244,162,97,0.5)] hover:scale-[1.05] active:scale-95 transition-all border-none group">
                        <Link href="#pricing" className="flex items-center gap-3 md:gap-5">
                            Deploy System Now <ArrowRight className="w-8 h-8 md:w-10 md:h-10 transition-transform group-hover:translate-x-3" />
                        </Link>
                    </button>
                </div>
            </Section>
