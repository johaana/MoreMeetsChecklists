    ...
    <section id={id} className="relative w-full min-h-screen flex flex-col justify-center overflow-hidden bg-black border-b border-white/10">
        ...
        <div className="container px-6 relative z-10 mx-auto">
            <div className="max-w-xl relative group"> 
                {/* Technical Atmospheric Glow */}
                <div className={cn("absolute -inset-10 rounded-[4rem] blur-3xl opacity-20 group-hover:opacity-40 transition-opacity duration-1000 animate-pulse-soft", glowClass)} />
                
                {/* High-Gravity Frosted Glass Card */}
                <div className="relative z-10 p-10 md:p-16 rounded-[3.5rem] border border-white/20 border-t-white/60 border-l-white/60 bg-white/85 backdrop-blur-3xl space-y-10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-500">
                    <div className="space-y-8">
                        <div className="w-20 h-3 rounded-full opacity-80" style={{ backgroundColor: accentBg }} />
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-lg md:text-xl text-zinc-600 italic font-medium leading-relaxed border-l-4 border-zinc-200 pl-8">
                        ...
                        <div className="space-y-3 pt-2">
                            {SYSTEM_SPECS.map((spec, i) => (
                                <div key={i} className="flex items-center gap-4">
                                    ...
                                    <span className="text-[11px] font-black uppercase tracking-[0.2em] text-zinc-500 italic">{spec}</span>
                                </div>
                            ))}
                        </div>
                    </div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-center text-white/40">
...
const TaskSnippetGlass = ({ title, audit, action, risk, accentColor }: any) => (
    <div className="p-10 rounded-[3rem] border border-white/20 border-t-white/60 border-l-white/60 bg-white/90 backdrop-blur-3xl shadow-xl space-y-8 hover:bg-white/95 transition-all group flex flex-col justify-between h-full">
        <div className="space-y-6">
            <div className="flex items-center gap-5">
                <div className={cn("p-3 rounded-2xl bg-zinc-100/50 border border-white/20", accentColor)}>
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white">
            ...
            <main className="flex-1">
                ...
                {/* OPTION 1: PLATINUM EXECUTIVE (As per screenshot) */}
                <HeroVariation 
                    id="platinum"
                    badge="1. Platinum Executive (Prestige)"
                    glowClass="bg-amber-500/30"
                    accentText="#f5a623"
                    accentBg="#f5a623"
                />

                {/* OPTION 2: INSTITUTIONAL SKY (Trust Standard) */}
                <HeroVariation 
                    id="sky"
                    badge="2. Institutional Sky (Trust)"
                    glowClass="bg-blue-500/30"
                    accentText="#2563eb"
                    accentBg="#2563eb"
                />

                {/* OPTION 3: MEDICAL EMERALD (Clinical Standard) */}
                <HeroVariation 
                    id="emerald"
                    badge="3. Medical Emerald (Authority)"
                    glowClass="bg-emerald-500/30"
                    accentText="#059669"
                    accentBg="#059669"
                />

                {/* OPTION 4: MIDNIGHT SLATE (Corporate Standard) */}
                <HeroVariation 
                    id="slate"
                    badge="4. Midnight Slate (Governance)"
                    glowClass="bg-indigo-500/30"
                    accentText="#4f46e5"
                    accentBg="#4f46e5"
                />

                {/* OPTION 5: INDUSTRIAL COPPER (Stability Standard) */}
                <HeroVariation 
                    id="copper"
                    badge="5. Industrial Copper (Stability)"
                    glowClass="bg-orange-500/30"
                    accentText="#ea580c"
                    accentBg="#ea580c"
                />

                {/* HIGH-CONTRAST COMPONENT LIBRARY */}
                <section className="py-24 md:py-32 bg-zinc-50 relative overflow-hidden">
                    ...
                    <div className="container px-6 mx-auto relative z-10">
                        <div className="max-w-4xl mx-auto text-center space-y-4 mb-20">
                            <Badge variant="outline" className="uppercase tracking-[0.4em] font-black text-[10px] border-zinc-200 text-zinc-400 px-6 py-1.5 rounded-full">Universal Glass Hub</Badge>
                            <h2 className="text-4xl md:text-7xl font-black font-headline uppercase italic tracking-tighter text-zinc-900 leading-[0.9]">Audit-Ready <br /> Execution</h2>
                            <p className="text-lg md:text-xl text-zinc-500 italic font-medium max-w-xl mx-auto leading-relaxed">High-gravity technical snippets engineered for ocular comfort and professional authority.</p>
                        </div>
                        ...
                        <div className="mt-20 max-w-5xl mx-auto p-12 md:p-20 rounded-[4rem] border border-white/20 border-t-white/60 border-l-white/60 bg-white/90 backdrop-blur-3xl shadow-2xl space-y-16 transition-all hover:bg-white/95">
                            <div className="flex flex-col md:flex-row items-center justify-between gap-12 border-b border-zinc-200/50 pb-12">
                                ...
                                <div className="flex items-center gap-8 bg-zinc-100/50 p-4 rounded-[2rem] border border-white/40">
                                    <div className="text-center px-8 border-r border-zinc-300">
                            ...
                            <div className="grid md:grid-cols-3 gap-12">
                                {[
                                    ...
                                    <div key={i} className="flex gap-6 group">
                                        <div className="w-14 h-14 rounded-2xl bg-zinc-100 border border-white/20 flex items-center justify-center text-zinc-400 shrink-0 group-hover:bg-primary/10 group-hover:text-primary transition-all shadow-sm">
