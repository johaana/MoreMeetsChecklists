...
// --- CONFIGURATION: MINIMAL HIGH-CONVERSION NARRATIVE (V96.0) ---
...
const StaticVideo = React.memo(({ opacity = 0.4, blur = 0 }: { opacity?: number, blur?: number }) => (
    <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-white" style={{ transform: 'translate3d(0,0,0)' }}>
        <video 
            ...
            playsInline 
            preload="auto"
            className="w-full h-full object-cover will-change-transform grayscale-[0.1]" 
            style={{ opacity, filter: `blur(${blur}px)`, transform: 'translate3d(0,0,0)' }}
        />
        <div className="absolute inset-0 bg-gradient-to-tr from-white/20 via-transparent to-white/10 z-10" />
    </div>
));
...
const FixedCommandFloor = ({ accentColor = "#00AE8D" }: { accentColor?: string }) => (
    <div className="space-y-8 pt-6 relative z-30">
        {/* 1. Subtext */}
        <p className="text-base md:text-lg font-medium leading-relaxed text-[#475569]/80 italic max-w-md">
            {SUBTEXT}
        </p>
        ...
        <div className="space-y-5">
            <Button asChild size="lg" style={{ background: accentColor }} className={cn(
                ...
                "hover:brightness-110 hover:-translate-y-1 active:scale-95 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)]"
            )}>
            ...
            {/* 3. Micro-Boosters */}
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[9px] md:text-[10px] font-black uppercase tracking-[0.25em] text-[#475569]/50">
        ...
        <div className="pt-6 border-t border-black/5 w-fit">
            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] text-[#475569]/30 italic">
...
export default function DesignLabPage() {
    ...
    const archetypes = [
        { id: 6, label: "ASEGO TEAL", accent: "#00AE8D" },
        { id: 1, label: "PEACH GLOW", accent: "#FF8A71" },
        { id: 2, label: "AZURE HUD", accent: "#0EA5E9" },
        { id: 3, label: "GOLD STANDARD", accent: "#EAB308" },
        { id: 4, label: "VIVID SPLIT", accent: "#00AE8D" }
    ];
        ...
        <div className="flex flex-col min-h-screen selection:bg-[#00AE8D]/20 overflow-hidden font-body bg-white">
            ...
            <main className="flex-1">
                ...
                <div className="fixed top-24 right-8 z-50 flex flex-col gap-2 p-2 bg-white/80 backdrop-blur-xl rounded-2xl border border-zinc-200 shadow-2xl">
                    <span className="text-[8px] font-black text-center text-zinc-400 uppercase tracking-widest pb-1 border-b border-zinc-100 mb-1">SOVEREIGN V96</span>
                    {archetypes.map((arch) => (
                        <button 
                            key={arch.id} 
                ...
                {/* --- ALL ARCHETYPES USE THE GLASS SLAB LOGIC --- */}
                <section className="relative w-full h-screen flex items-center overflow-hidden">
                    <StaticVideo opacity={archetype === 4 ? 0.8 : 0.45} blur={archetype === 2 ? 4 : 0} />
                    
                    <div className={cn(
                        "container relative z-20 flex h-full px-6 md:px-24 mx-auto",
                        archetype === 3 ? "items-center justify-center" : "items-center"
                    )}>
                        <div className={cn(
                            "max-w-3xl w-full p-10 md:p-16 rounded-[2.5rem] border border-white/40 shadow-2xl relative overflow-hidden group",
                            "bg-white/70 backdrop-blur-2xl",
                            archetype === 1 && "border-[#FF8A71]/20",
                            archetype === 2 && "bg-white/60 border-[#0EA5E9]/20",
                            archetype === 3 && "max-w-2xl text-center items-center flex flex-col",
                            archetype === 4 && "ml-auto mr-0 md:mr-12"
                        )}>
                            {/* Rim Lighting Accent */}
                            ...
                            <div className="space-y-4">
                                <h1 className="text-4xl md:text-6xl font-black font-headline text-[#0F172A] leading-[1] tracking-tighter uppercase italic">
                                    {STATIC_HEADLINE}
                                </h1>
                                <div className={cn("min-h-[40px] flex items-center", archetype === 3 && "justify-center")}>
                            ...
                            <div className={cn("w-full flex flex-col", archetype === 3 && "items-center")}>
