...
/**
 * SOVEREIGN V3.2.1 PILOT-READY ENGINE - PHASE 1: SAFE PARITY (FINAL)
 * ----------------------------------------------------------------------------
 * 1. IDENTITY LOCK (P0): Frozen Top 3 Rows.
 * 2. CONTENT PRESERVATION: Fully implements SOP_LIB, DASHBOARD, etc.
...
export const handleDownloadAuditPrototype = (item: PremiumPack = hotels_and_resorts) => {
    try {
        ...
        const startData = [
            ...
            [],
            [{ v: "⚠️ SAMPLE DATA NOTICE", s: { font: { bold: true, color: { rgb: "64748B" } } } }],
                ...
                if (roleChecklist) {
                    roleChecklist.tasks.forEach(t => {
                        ...
                        taskData.push([
                            ...
                            { v: t.floorAction || t.trainerNotes || t.description || "", s: { ...styles.left, font: { color: { rgb: "065F46" } } } }
                        ]);
                    });
                }
            });
        ...
        addSheetHeader(taskWs, TABS.DAILY_TASKS, "Update 'Done By' to complete daily work.");
