...
const HeroSection = () => (
    <section className="relative w-full flex items-center text-white overflow-hidden min-h-screen">
        ...
        <div className="container px-4 md:px-6 relative z-20 flex flex-col justify-center min-h-screen py-16">
            <div className="max-w-[560px] space-y-6 text-left">
                ...
                <div className="space-y-2 text-lg text-secondary-text">
                    <p>People forget <span className="text-red-400">during audits.</span></p>
                    <p>During <span className="text-red-400">shift changes.</span></p>
                    <p>During <span className="text-red-400">emergencies.</span></p>
                    <p>During <span className="text-red-400">growth.</span></p>
                </div>

                <p className="text-lg text-secondary-text leading-relaxed max-w-xl">
                    That’s when steps get skipped — and small misses become expensive failures.
                </p>

                <p className="font-medium text-lg text-primary-text !mt-8">
                    If work lives in people’s heads, it will fail under pressure.
                </p>

                <p className="text-secondary-text leading-relaxed max-w-xl">
                    MoreMeets hard-codes how work is done — so execution doesn’t depend on memory, experience, or who’s on shift.
                </p>
                
                <div className="pt-4 flex flex-col items-start gap-2">
                    <Button size="lg" asChild className="group h-auto" style={{ backgroundColor: '#F5C94A', color: '#111816', fontWeight: 600, padding: '16px 24px', borderRadius: '6px' }}>
                        <Link href="/library">
                            See how standards are enforced <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                        </Link>
                    </Button>
                    <p className="text-sm text-muted-text">Built for audited, regulated, multi-location operations</p>
                </div>
            </div>
        </div>
    </section>
...
const HowItBreaksSection = () => {
    const failurePoints = [
        { title: "A task no one knew was due", description: "A critical compliance check is missed for weeks because the one person who remembered it is on leave." },
        { title: "A new hire doing it “their way”", description: "A new team member, lacking clear guidance, improvises a safety procedure, creating a massive liability." },
        { title: "A critical step skipped during a rush", description: "Under pressure, a veteran employee skips a key quality control step they've done a thousand times. The result is a defective batch." },
        { title: "“We’ve always done it this way”", description: "An outdated process remains in use because it's 'tribal knowledge', exposing the company to modern risks." }
    ];
